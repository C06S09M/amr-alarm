...
5. `APP_PASSWORD`를 설정했다면 앱 주소 접속 후 대시보드 비밀번호로 로그인.
...
배포 환경에서는 반드시 `APP_PASSWORD`를 설정하세요. 설정하지 않으면 하위 호환을 위해 로그인 없이 대시보드가 열립니다. 로그인 세션은 서버 메모리에 보관되며 서버 재시작 시 만료됩니다.
