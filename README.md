...
`android/` 폴더에 Android Studio용 Kotlin 앱 프로젝트가 있습니다. 이 앱은 AMR 대시보드를 WebView로 열고, Android 알림 접근 서비스로 카카오톡·문자 알림을 직접 `/api/ingest`에 전송합니다. 따라서 네이티브 앱에서는 MacroDroid 없이도 카카오톡·문자 캡처를 받을 수 있습니다.

Android Studio에서 `android/` 폴더를 열고 `android/README.md`의 순서대로 실행하세요. Android SDK와 Gradle이 설치되지 않은 환경에서는 이 저장소에서 APK를 직접 빌드할 수 없습니다.

> 개인용 APK에 서버 수신 토큰이 포함되어 있습니다. APK를 타인에게 배포할 경우에는 사용자별 인증 토큰 구조로 바꾸는 것이 필요합니다.
